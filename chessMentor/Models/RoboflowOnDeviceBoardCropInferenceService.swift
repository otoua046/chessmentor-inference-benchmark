...
enum BoardCropperInferenceBackend {
    case hosted
    case onDevice
}

final class HostedBoardCropInferenceService: BoardDetectionServing {
    private let client: RoboflowClient
    ...
    func detect(on image: UIImage) async throws -> [Prediction] {
        try await client.detect(on: image)
    }
}

final class RoboflowOnDeviceBoardCropInferenceService: BoardDetectionServing {
    ...
    convenience init(
        ...
        let model = Self.parseModelID(boardModelId)
        self.init(
